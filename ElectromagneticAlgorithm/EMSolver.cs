...
namespace ElectromagneticAlgorithm
{
    public class EMSolver
    {
        ...
        public static void PMX(IPermutationSolution solution1, IPermutationSolution solution2, int l, int r)
        {
            ...
            List<int> s1 = new List<int>(solution1.GetSolutionRepresentation());
            List<int> s2 = new List<int>(solution2.GetSolutionRepresentation());
            List<int> s1Copy = new List<int>(s1);
            int range = r - l;
            if (range <= 0) throw new Exception(); // TODO: Custom exception
